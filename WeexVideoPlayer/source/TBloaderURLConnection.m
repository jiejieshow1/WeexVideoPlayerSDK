...
- (void)dealWithLoadingRequest:(AVAssetResourceLoadingRequest *)loadingRequest
{
    ...
    if (!self.task) {
        ...
        [self.task setUrl:interceptedURL offset:0 WithHeader:self.urlHeader];
    } else {
        ...
        if (self.task.offset + self.task.downLoadingOffset + 1024 * 300 < range.location ||
            ...
            [self.task setUrl:interceptedURL offset:range.location WithHeader:self.urlHeader];
        }
    }
...
- (NSURL *)getSchemeVideoURL:(NSURL *)url
{
    ...
    self.urlHeader = components.scheme;
