...
- (void)setUrl:(NSURL *)url offset:(NSUInteger)offset WithHeader:(NSString *)header
{
    _url = url;
    _offset = offset;
    ...
    actualURLComponents.scheme = header;
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if (self.taskArr.count < 2) {
        ...
        NSString *document = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES).lastObject;
        NSString *movePath =  [document stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.mp4",actualURLComponents.query]];
