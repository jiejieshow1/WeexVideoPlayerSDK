...
//
//// github地址：https://github.com/suifengqjn/BGWeexVideoPlayer
...
NSString *const BLBPlayerStateChangedNotification    = @"BLPlayerStateChangedNotification";
NSString *const BLBPlayerProgressChangedNotification = @"BLPlayerProgressChangedNotification";
NSString *const BLBPlayerLoadProgressChangedNotification = @"BLPlayerLoadProgressChangedNotification";


@interface BGWeexVideoPlayer ()<TBloaderURLConnectionDelegate>
...
@implementation BGWeexVideoPlayer
...
- (void)playWithUrl:(NSURL *)url showView:(UIView *)showView
{
    ...
    if (IOS_VERSION < 7.0 || ![str hasPrefix:@"http"]) {
        ...
        self.currentPlayerLayer.frame = CGRectMake(0, 0, showView.bounds.size.width, showView.bounds.size.height-44);
        _isLocalVideo = YES;
    ...
    if ([url.scheme isEqualToString:@"file"]) {
        ...
        if (self.state == TBPlayerStatePlaying) {
            [[NSNotificationCenter defaultCenter] postNotificationName:BLBPlayerStateChangedNotification object:nil];
        } else {
    ...
    if (!_navBar) {
        ...
        _navBar.frame = CGRectMake(0, showView.bounds.size.height-44, showView.bounds.size.width, 44);
...
//{
//    _navBar.hidden = YES;
//    self.showView.layer.transform = CATransform3DMakeRotation(M_PI/2, 0, 0, 1);
...
{
    NSLog(@"-- %lf -- %lf --",playerItem.duration.value,playerItem.duration.timescale);
...
- (void)setLoadedProgress:(CGFloat)loadedProgress
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:BLBPlayerLoadProgressChangedNotification object:nil];
}
...
- (void)buildVideoNavBar
{
    ...
    _navBar.frame = CGRectMake(0, _showView.bounds.size.height-44, _showView.bounds.size.width, 44);
    ...
    _videoProgressView.frame = CGRectMake(62+30, _navBar.bounds.size.height-21, kScreenWidth-124-44, 20);
    ...
    CGFloat progressviewY = _videoProgressView.center.y;
    CGPoint sliderframe = self.playSlider.center;
    sliderframe.y = progressviewY;
    self.playSlider.center = sliderframe;
    ...
    _stopButton.frame = CGRectMake(0, _navBar.bounds.size.height-44, 44, 44);
...
//    }
//    _screenBUtton.frame = CGRectMake(_navBar.bounds.size.width - 40, 0, 44, 44);
